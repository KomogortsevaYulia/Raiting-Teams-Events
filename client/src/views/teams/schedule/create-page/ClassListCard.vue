    ...
    <div class="class-list-card__content">
      <div v-if="!disabled" class="class-list-card__list">
        ...
        <div class="list__row" v-for="(el, i) in list" :key="i">
          <span
            class="list__row__star"
            :class="{ selected: el.starred }"
            @click="toggleStar(el)"
          >
          ...
          <!--   auditory-->
          <div
            class="row-card btn-custom-neutral"
            @click="onAuditoryClick(el)"
            type="button"
...
import SubmitModal from "@/views/teams/schedule/create-page/SubmitModal.vue";
import { ref } from "vue";
...
const selectedAuditory = ref<ClassListCardList>();

function toggleStar(el: ClassListCardList) {
  el.starred = !el.starred;
}

function onAuditoryClick(el: ClassListCardList) {
  selectedAuditory.value = el;
}
</script>
