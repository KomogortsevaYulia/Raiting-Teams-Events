...
<template>
  <div class="d-grid gap-2 p-4 rounded shadow">
    <div class="row justify-content-between">
      <div class="col-auto">
        <b>Свободны <span class="badge rounded-pill bg-danger">02 октября</span> в
          <span class="badge rounded-pill bg-danger">14:00</span></b>
      </div>
      <div class="col-auto">
        <FontAwesomeIcon icon="x" class="mx-2"/>
      </div>
    </div>
    <div class="dropdown">
      <div
    ...
    <div>
      <button type="button" class="btn btn-light m-1" v-for="auditorium in auditoriumList" :key="auditorium.id">
        {{ auditorium.name }} <span class="badge bg-light text-dark">{{ auditorium.capacity }}</span>
      </button>
    </div>
  </div>
</template>
